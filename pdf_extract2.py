...
import log
import logging
pdf = pdfplumber.open('C:/Dev/cass rpa/April/AprP2_lhr.pdf')
user = "kskarzynski"
pwd= "zar519chr!"
cnxn = pyodbc.connect("DRIVER={SQL Server};SERVER=10.2.115.129;DATABASE=devdb;UID="+(user)+";PWD="+(pwd))
l=log.Log()
...
def cost_control(shipment_id):
    ...
    """
    df = pandas.read_sql_query(sqlcommand, con=cnxn)
...
def parse_row_airline(first_line):
        return {
            "prefix": first_line[:4].strip()
        }

def parse_row(first_line):
...
for x in range(0,len(pdf.pages)):
    try:
        ...
        p = [ parse_row_airline(first_line) 
            for first_line in line ]
        parsed = [ parse_row(first_line) 
            for first_line in lines ]

        columns = list(parsed[0].keys())
        # print (p[0]['prefix'])
        df = pd.DataFrame(parsed)[columns]
        df.loc[df["amount"].str.endswith("-"), "amount"] = ("-" + df.loc[df["amount"].str.endswith("-"), "amount"].str.strip("- "))
        ...
        for index, row in df.iterrows(): 
            df.at[index,'cost_list'] = cost_control(row['shipment_id'])
        for index, row in df.iterrows():
            df.at[index,'match'] = best_match(row['cost_list'],row['amount'])
        for index, row in df.iterrows():
...
df1.to_csv('C:/Dev/cass rpa/April/AprP2_lhr.csv')
print (df)
#A11234724
